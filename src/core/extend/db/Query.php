...
##version
use \Exception;
use bear\extend\db\ArrayAccess;
use bear\sys\config\Config;
use bear\extend\dir\Dir;
use bear\extend\page\Page;
...
class Query implements \ArrayAccess, \Iterator
{
    ...
    {
        //SQL数据库连接引擎
        $class            = 'bear\\extend\\db\\connection\\'.$driver;
    ...
    {
        $buile       = 'bear\\extend\\db\\build\\'.$driver;
    ...
    {
        //目录检测
        $dir = Config::get('bear.bear_dir').Config::get('database.cache_dir');
        Dir::create($dir);
        //缓存文件
        $file = $dir.'/'.($name).'.php';
        //读取数据
     ...
     * [render 格式化分页处理]
     * @Author   Jerry
     * @DateTime 2018-09-26T14:12:30+0800
     * @Example  eg:
     * @return   [type]                   [description]
     */
    public function render($row=10, $pageNum = 8){
        $obj = unserialize(serialize($this));
        return  Page::row($row)->pageNum($pageNum)->make($obj->count())->show();
        // Page::row($row)->pageNum($pageNum)->make($obj->count());
    }
    ...
    public function where()
    {
        ...
        if (isset($args[0])&&is_array($args[0])) {
            ...
            switch (count($args)) {
                case 1:
                    if($args[0])$this->logic('AND')->build->bindExpression('where', $args[0]);
                    break;
                case 2:
